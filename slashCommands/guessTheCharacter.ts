...
function gtcSearch(name: string) {
...
function msgFilter(response: string, choice: number) {
...
const exportCommand: SlashCommand = {
    name: 'guess',
    async execute({ interaction, author, locale, server, customFlag, reply, warn }) {

        const difficulty = interaction.options.getString('difficulty') ?? "easy";
        const isPrivate = interaction.options.getBoolean('private') ?? false;

        let charArray: charInfo[] = [];
        ...
        return interaction[interaction.replied ? "followUp" : "reply"]({ embeds: [Embed], components: [row] }).then((emsg) => {
                    ...
                    interaction.awaitModalSubmit({ filter: (r) => r.customId === ('gtc_modal' + uid), time: 60000 }).then(async modalInteraction => {
                        ...
                        if (!msgFilter(response, pick.id)) {
                            ...
                            const Embed = new EmbedBuilder()
                                ...
                                .setFooter({ text: `${modalInteraction.user.tag}`, iconURL: modalInteraction.user.displayAvatarURL({ size: 2048 }) });
